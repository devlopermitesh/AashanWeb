...
export function AddressPicker({
  ...
  const didInitMarkerRef = useRef(false)
  ...
  useEffect(() => {
    ...
    if (selected) return
    setQuery((prev) => (prev ? prev : initialQuery))
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      didInitMarkerRef.current = false
      setIsMapReady(false)
    }
  ...
  useEffect(() => {
    ...
    if (didInitMarkerRef.current) return
    if (!initialMarker) return

    const map = mapInstanceRef.current
    if (!map) return

    didInitMarkerRef.current = true
    placeMarkerOnMap(map, defaultLat, defaultLng, null)
    if (reverseGeocodeOnLoad) {
      reverseGeocode(map, defaultLat, defaultLng)
    }
  }, [
    defaultLat,
    defaultLng,
